...
public class GestorToken {
	...
	private static GestorToken instance = new GestorToken();
	 ...
	 * @return la instancia
	 */
	public static GestorToken getInstance() {
		return instance;
	}
	...
	public TokenOD BuscarIP(TokenOD Token) {
		...
		if (result != null) {
			...
			System.err.println("Ip que regresa de la base de dato  "+ Token.getToken());

			System.err.println("Ip que regresa de la base de dato  "+ Token.getIp());
			return token.buscarIP(Token);
		} else {
			System.err.println("No encontre nada ");
			return null;
		}

	}

	public TokenOD BuscarToken(UsuarioOD usuario) {
		System.err.println("este es el nikc para buscarle su token"
				+ usuario.getNick());
		TokenDAO token = new TokenMongoDB();
		TokenOD result = token.buscarPorUsuario(usuario);
		if (result != null) {
			...
			try {
				// Date today = df.parse(aux.getFecha());
				String fecha = result.getFecha().split(" ")[3];
				int hh = Integer.parseInt(fecha.split(":")[0]);
				int mm = Integer.parseInt(fecha.split(":")[1]);
				int ss = Integer.parseInt(fecha.split(":")[2]);
				// Date fechaG = new Date();
			...
			System.out.println("diferencia" + dif);
			if (dif <= 300000) {
				Logger.info("GestorToken: Token :" + result.getToken()
						+ "del usuario " + usuario.getNick() + "vigente");
				return result;
			} else {
				Logger.error("GestorToken: Token vencido");
				return null;
			}

		} else {
			System.err.println("No encontre nada ");
			Logger.error("GestorToken: El usuario: " + usuario.getNick()
					+ "no tiene token");
			return null;
		}
